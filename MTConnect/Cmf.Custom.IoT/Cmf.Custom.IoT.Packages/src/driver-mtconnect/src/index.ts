...
import { handleCompiledAddons, ConfigurationSectionDriver, CompiledAddonsSettings, findPackageRootDirectory } from "@criticalmanufacturing/connect-iot-driver";
...
if (yargs.argv) {
    ...
    logger.setIdentificationTokens({
        ...
        applicationName: "DriverMTConnect",
        pid: process.pid,
        componentId: argv.componentId || "DriverMTConnect",
    ...
    logger.info(`Starting MTConnect driver with pid "${process.pid}".`);
    logger.debug(`Command line: ${Utils.objectToString(yargs.argv)}`);
    logger.info(`  ConfigurationFile='${configurationFile}'`);

    const addonConfiguration: CompiledAddonsSettings = {
        addonsRootLocation: ["addons"],
        controlFileName: "_currentVersion.txt",
        addons: [
            { name: "EdgeJs", specific: true, destination: ["lib", "native"] },
        ]
    };
    if (handleCompiledAddons(addonConfiguration)) {
        // Addons were processed, so this means the driver is running with the packer in place. Must change the references
        // Must do this to avoid being replaced with "virtual" location
        const baseSrc: string = findPackageRootDirectory();

        const nativeModule = "edge_coreclr.node";

        process.env["EDGE_NATIVE"] = path.join(baseSrc, "lib", "native", nativeModule);
        logger.info(`Using .net (Core or Native) addon located in '${process.env["EDGE_NATIVE"]}'`);
        process.env["EDGE_BOOTSTRAP_DIR"] = path.join(baseSrc, "addons", "EdgeJsBootstrap");
        logger.info(`Using Edge bootstrap located in '${process.env["EDGE_BOOTSTRAP_DIR"]}'`);
    }
    ...
    Runner.run({
        ...
        logger.info(`MTConnect Driver process started with success`);
    }).catch((error: Error) => {
        logger.error(`MTConnect Driver process failed to start!`);
