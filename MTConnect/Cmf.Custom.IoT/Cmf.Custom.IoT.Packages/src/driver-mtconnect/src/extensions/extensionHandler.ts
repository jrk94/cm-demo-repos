...
import { TYPES as DRIVER_TYPES, Command, CommandParameter } from "@criticalmanufacturing/connect-iot-driver";
...
export class ExtensionHandler {
    ...
    private _driver: MTConnectDeviceDriver;
    ...
    public async initialize(container: Container): Promise<void> {
        ...
        this._driver = this._container.get<MTConnectDeviceDriver>(DRIVER_TYPES.Device.Driver);
            ...
            if (msg.content.parameters) {
                for (const p in msg.content.parameters) {
                    if (msg.content.parameters.hasOwnProperty(p)) {
                        if (p !== "$id") {
                            const param = command.parameters.find(cp => cp.name === p);
                            if (param != null) {
