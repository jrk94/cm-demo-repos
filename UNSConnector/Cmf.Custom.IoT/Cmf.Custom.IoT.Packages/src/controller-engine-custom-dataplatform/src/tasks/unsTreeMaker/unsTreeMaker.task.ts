    ...
    public override async onChanges(changes: Task.Changes): Promise<void> {
        if (changes["activate"]) {
            try {
                ...
                const enterpriseName = (rawData?.Enterprise?.Name?.trim() || defaultEnterprise).replace(/ /g, "-");
                ...
                // Build topic levels
                const enterpriseTopic = `CM${separator}${enterpriseName}`;
                const siteTopic = `${enterpriseTopic}${separator}${siteName}`;
                ...
                this._logger.warning(`result: ${JSON.stringify(result)}`);
