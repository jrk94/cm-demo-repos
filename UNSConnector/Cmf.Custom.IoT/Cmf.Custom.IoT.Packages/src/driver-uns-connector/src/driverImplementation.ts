...
import { UNSConnectorCommunicationSettings, uNSConnectorDefaultCommunicationSettings, validateCommunicationParameters } from "./communicationSettings";
...
@injectable()
export class UNSConnectorDeviceDriver extends DeviceDriverBase {
    private _communicationSettings: UNSConnectorCommunicationSettings;
    private _container: Container;
    private _subscriber: MQTTSubscriber;
    ...
    public async initializeDriver(): Promise<void> {
        if (!this._container) {
            ...
            this._subscriber = this._container.get<MQTTSubscriber>(TYPES.MqttSubscriberHandler);
    ...
    public async setCommunicationConfiguration(communication: any): Promise<void> {
        this._communicationSettings = Object.assign({}, uNSConnectorDefaultCommunicationSettings, communication);
    ...
    public async connectToDevice(): Promise<void> {
        ...
        try {
            ...
            // Connect to the equipment
            await this._subscriber.setup(this._communicationSettings, this.configuration.events);
            this._subscriber.on("onSubscriberData", this.onSubscriberData.bind(this));
            this._subscriber.on("onSubscriberDataMultiple", this.onSubscriberDataMultiple.bind(this));
    ...
    */
    ...
    private async onSubscriberData(eventToPost: EventToPost): Promise<void> {
        this.onEventOccurrence(eventToPost.eventId, eventToPost.eventValues);
    }

    private async onSubscriberDataMultiple(eventsToPost: EventToPost[]): Promise<void> {

        const eventOccurrences: EventOccurrence[] = [];
        for (const eventToPost of eventsToPost.filter(e => this.configuration.events.find(ce => ce.name === e.eventId && ce.isEnabled))) {

            const values = eventToPost.eventValues;
            const event: EquipmentEvent = this.configuration.events.find(e => e.name === eventToPost.eventId)!;
            const results: PropertyValue[] = [];
            ...
            if (values) {
                for (const eventProperty of event.properties) {
                    if (values.has(eventProperty.name)) {
                        const value: any = values.get(eventProperty.name);
                        ...
                        results.push(propertyValue);
                    } else if (eventProperty.extendedData?.isMandatory) {
                        this.logger.warning(`Mandatory Value for property '${eventProperty.name}' was not received in the event data for event '${event.name}'. Skipping event.`);
                        missingMandatory = true;
                        break;
                    }
                    else {
                        this.logger.debug(`Value for property '${eventProperty.name}' was not received in the event data for event '${event.name}'`);
                    }
                }
            }
            ...
            eventOccurrences.push({
                timestamp: new Date(),
                eventDeviceId: event.extendedData?.iotEventName || event.name,
                eventName: event.name,
                eventSystemId: event.systemId,
                propertyValues: results,
                path: event.extendedData?.path
            } as any);
        }

        if (eventOccurrences.length > 0) {
            this.emit("eventOccurrence", {
                timestamp: new Date(),
                bulk: eventOccurrences,
                eventDeviceId: "Bulk",
                eventName: "Bulk",
                eventSystemId: "Bulk",
                propertyValues: [{
                    propertyName: "Bulk",
                    originalValue: eventOccurrences,
