...
if (yargs.argv) {
    ...
    logger.setIdentificationTokens({
        ...
        applicationName: "DriverAMQP",
        pid: process.pid,
        componentId: argv.componentId || "DriverAMQP",
    ...
    logger.info(`Starting AMQP driver with pid "${process.pid}".`);
    ...
    Runner.run({
        ...
        logger.info(`AMQP Driver process started with success`);
    }).catch((error: Error) => {
        logger.error(`AMQP Driver process failed to start!`);
