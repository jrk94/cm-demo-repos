...
namespace IndustrialEquipmentSimulator
{
    public partial class ScenarioRunner
    {
        ...
        {
            Console.WriteLine($"Starting Finished Good Process");
            ...
            Console.WriteLine($"Finishing Finished Good Process");
            ...
            Console.WriteLine($"Shipped Finished Goods");
        }
        private async Task<Material> BaseAsm(Material lot, decimal speed = 1)
        {
            ...
            lot = await MaterialTracking(lot, resourceName, "HeatModCompTub", speed, [15, 40], "InduTech State Model", postTrackInAction: async (material, resourceName) =>
            {
        ...
        private async Task<Material> HeatModCompTub(Material lot, decimal speed = 1)
        {
            ...
            lot = await MaterialTracking(lot, resourceName, "HeatModCompTub", speed, [15, 40], "InduTech State Model");
        ...
        private async Task<Material> InstallOdCoilTubes(Material lot, decimal speed = 1)
        {
            ...
            lot = await MaterialTracking(lot, resourceName, "InstallOdCoilTubes", speed, [15, 40], "InduTech State Model", postTrackInAction: async (material, resourceName) =>
            {
                await System.Threading.Tasks.Task.Delay(executionTime);
                return ManualAssembly(lot);
            });
        ...
        private async Task<Material> NitrogenEvacCharge(Material lot, decimal speed = 1)
        {
            ...
            lot = await MaterialTracking(lot, resourceName, "NitrogenEvacCharge", speed, [15, 40], "InduTech State Model", postTrackInAction: async (material, resourceName) =>
            {
                await System.Threading.Tasks.Task.Delay(executionTime);
                return ManualAssembly(lot);
            });
        ...
        private async Task<Material> ApplyFoam(Material lot, decimal speed = 1)
        {
            ...
            lot = await MaterialTracking(lot, resourceName, "ApplyFoam", speed, [15, 40], "InduTech State Model",
        ...
        private async Task<Material> FinalWiring(Material lot, decimal speed = 1)
        {
            ...
            lot = await MaterialTracking(lot, resourceName, "FinalWiring", speed, [15, 40], "InduTech State Model",
        ...
        private async Task<Material> InstallCoverDoorsFansAndRoof(Material lot, decimal speed = 1)
        {
            ...
            lot = await MaterialTracking(lot, resourceName, "InstallCoverDoorsFansAndRoof", speed, [15, 40], "InduTech State Model",
                postTrackInAction: async (material, resource) =>
                {
                    await System.Threading.Tasks.Task.Delay(executionTime);
                    return ManualAssembly(lot);
        ...
        private async Task<Material> CleanUpAndInspection(Material lot, decimal speed = 1)
        {
            ...
            lot = await MaterialTracking(lot, resourceName, "CleanUpAndInspection", speed, [15, 40], "InduTech State Model");
        ...
        private async Task<Material> RTUPacking(Material lot, decimal speed = 1)
        {
            ...
            lot = await MaterialTracking(lot, resourceName, "RTUPacking", speed, [15, 40], "SEMIE10",
                ...
                postTrackInAction: async (material, resourceName) =>
                {
                    ...
                    {
                        int executionTime = Decimal.ToInt32(Random.Shared.Next(15, 40) * 1000 / speed);
        ...
        private async Task<Material> RTUFinalPacking(Material lot, decimal speed = 1)
        {
            ...
            lot = await MaterialTracking(lot, resourceName, "RTUFinalPacking", speed, [15, 40], "SEMIE10",
                postTrackInAction: async (material, resourceName) =>
                {
