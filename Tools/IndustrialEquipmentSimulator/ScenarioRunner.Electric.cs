...
namespace IndustrialEquipmentSimulator
{
    public partial class ScenarioRunner
    {
        ...
        private async Task<object> MainBoard(Material lot, decimal speed = 1)
        {
            ...
            lot = await MaterialTracking(lot, resourceName, "MainBoard", speed, [15, 40], "SEMIE10", postTrackInAction: async (material, resourceName) =>
            {
        ...
        private async Task<object> DoorPrep(Material lot, decimal speed = 1)
        {
            ...
            var employeeToCheckIn = this.employeeDefect.ElementAt(Random.Shared.Next(this.employeeDefect.Count)).Key;
            int executionTime = Decimal.ToInt32(new Random().Next(15, 40) * 1000 / speed);

            lot = await MaterialTracking(lot, resourceName, "DoorPrep", speed, [15, 40],
                preTrackInAction: (material, resourceName) =>
                {
                    CheckInOperator(resourceName, employeeToCheckIn, out resource, out _);
                ...
                {
                    CheckOutOperatorIfNoMaterialsInProcess(resourceName, employeeToCheckIn, out resource, out _);
                    return Task.FromResult(material);
                });
        ...
        private async Task<object> FilterPrep(Material lot, decimal speed = 1)
        {
            ...
            lot = await MaterialTracking(lot, resourceName, "FilterPrep", speed, [15, 40], "SEMIE10", postTrackInAction: async (material, resourceName) =>
            {
                await System.Threading.Tasks.Task.Delay(executionTime);
                return ManualAssembly(lot);
            });
        ...
        private async Task<object> StagingMB(Material lot, decimal speed = 1)
        {
            ...
            lot = await MaterialTracking(lot, resourceName, "StagingMB", speed, [15, 40], "SEMIE10", postTrackInAction: async (material, resourceName) =>
            {
                await System.Threading.Tasks.Task.Delay(executionTime);
                return ManualAssembly(lot);
            });
