...
namespace IndustrialEquipmentSimulator
{
    public partial class ScenarioRunner
    {
        ...
        private async Task<Material> Punch(Material lot, decimal speed = 1)
        {
            ...
            lot = await MaterialTracking(lot, resourceName, "Punch", speed, [15, 40], "SEMIE58",
        ...
        private async Task<Material> Bend(Material lot, decimal speed = 1)
        {
            ...
            lot = await MaterialTracking(lot, resourceName, "Bend", speed, [15, 40], "SEMIE58",
                preTrackInAction: (material, resource) =>
                {
                    this.ValidateOnlyMaterialsInProcessSameProduct(material, resource);
                    this.ValidateDurables(lot, resourceName);
        ...
        private async Task<Material> Coating(Material lot, decimal speed = 1)
        {
            ...
            var employeeToCheckIn = this.employeeDefect.ElementAt(Random.Shared.Next(this.employeeDefect.Count));

            lot = await MaterialTracking(lot, resourceName, "Coating", speed, [15, 40], "SEMIE58",
                preTrackInAction: (material, resource) =>
                {
                    this.ValidateOnlyMaterialsInProcessSameProduct(material, resource);
                    this.PrepareFeeders(resourceName);
                    CheckInOperator(resourceName, employeeToCheckIn.Key, out _, out _);
                    return Task.FromResult(material);
                },
                postTrackOutAction: (material, resourceName) =>
                {
                    CheckOutOperatorIfNoMaterialsInProcess(resourceName, employeeToCheckIn.Key, out _, out _);
                    return Task.FromResult(material);
                });
        ...
        private async Task<Material> PlatePainting(Material lot, decimal speed = 1, double defectProbability = 0.1)
        {
            ...
            var employeeToCheckIn = this.employeeDefect.ElementAt(Random.Shared.Next(this.employeeDefect.Count));
            defectProbability = employeeToCheckIn.Value;
            Console.WriteLine($"Selected {employeeToCheckIn.Key} at {resourceName} with defect probability of {defectProbability}");

            lot = await MaterialTracking(lot, resourceName, "PlatePainting", speed, [15, 40], "SEMIE58",
                preTrackInAction: (material, resource) =>
                {
                    this.ValidateOnlyMaterialsInProcessSameProduct(material, resource);
                    this.PrepareFeeders(resourceName);
        ...
        private async Task<Material> PlateColoring(Material lot, decimal speed = 1, double defectProbability = 0.1)
        {
            ...
            lot = await MaterialTracking(lot, resourceName, "Coloring", speed, [15, 40], "SEMIE58",
                preTrackInAction: (material, resource) =>
                {
                    this.ValidateOnlyMaterialsInProcessSameProduct(material, resource);
                    this.PrepareFeeders(resourceName);
        ...
        private async Task<(Material, bool)> HandleDefectWithRework(string typeOfDefect, decimal speed, Material material, Objects.Events events)
        {
            ...
            {
                Console.WriteLine($"Creating a defect {defectCharacteristic.Reason} for material {material.Name}");
            ...
            Console.WriteLine($"Created defects for material {material.Name}");
            ...
            try
            {
                Console.WriteLine($"Sending to Rework for material {material.Name}");
                ...
                Console.WriteLine($"Sent to Rework material {material.Name}");
            }
            catch (Exception ex)
            {
            ...
            material = await MaterialTracking(material, "Rework Station 1", "RW Polish", speed, [15, 40], "SEMIE58", preTrackOutAction: (material, resource) =>
            {
                ...
                Console.WriteLine($"Accepting defects for material {material.Name}");
                ...
                Console.WriteLine($"Accepted defects for material {material.Name}");

                return Task.FromResult(material);
            }, disableTrackin: true);

            // RW Inpection
            material = await MaterialTracking(material, "Rework Station 2", "RW Inspection", speed, [15, 40], "SEMIE10");
