...
```
frontend/
├── opcua-bridge/          # Node.js OPC UA to WebSocket bridge
│   └── src/
│       ├── index.ts       # Main server entry point
│       ├── opcua-client.ts # OPC UA client implementation
...
```bash
cd C:\cmf\cm-demo-repos\Tools\CNCOPCUASimulator
dotnet run
```

The simulator will start an OPC UA server at `opc.tcp://localhost:4840`.

### 2. Start the OPC UA Bridge

Open a new terminal and run:

```bash
cd C:\cmf\cm-demo-repos\Tools\CNCOPCUASimulator\frontend\opcua-bridge
...
```bash
cd C:\cmf\cm-demo-repos\Tools\CNCOPCUASimulator\frontend\cnc-dashboard
