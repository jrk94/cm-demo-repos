...
using IoTTestOrchestrator.OIB;
using IoTTestOrchestrator.OIB.ExtensionHandlers;
...
namespace SMTSimulator
{
    public partial class ScenarioRunner
    {
        ...
        {
            var pnpSimulator = (this._smtSimulators["PnP"] as IoTTestOrchestrator.OIB.PluginMain);
            ...
            var boarDataToBeSent = OIBUtilities.CreateBoardRequestData(panel.Name);
            boarDataToBeSent.Position.Line = DriverLineName;
            boarDataToBeSent.Position.Station = this.ppResources[0];

            BoardRequestResult result = (BoardRequestResult)pnpSimulator.SendMessage(IoTTestOrchestrator.OIB.SupportedExtensions.BoardGateKeeper, BoardGateKeeperSendMessage.BoardRequest.ToString(), boarDataToBeSent);

            if (result.RequestResult != "Confirmed")
            {
            ...
            var createdBoardProcessedData = OIBUtilities.CreateBoardProcessedData(panel.Name, this.ppResources[0]);

            pnpSimulator.SendMessage(SupportedExtensions.Eventing, EventingSendMessage.BoardProcessedEvent.ToString(), createdBoardProcessedData);

            _scenario.Utilities.WaitFor(60, $"Material for '{panel.Name}' never trackedout", () =>
            {
        ...
        {
            var pnpSimulator = (this._smtSimulators["PnPTemplate"] as IoTTestOrchestrator.OIB.PluginMain);
            ...
            var boarDataToBeSent = OIBUtilities.CreateBoardRequestData(panel.Name);
            boarDataToBeSent.Position.Line = DriverLineName;
            boarDataToBeSent.Position.Station = this.ppResources[1];

            BoardRequestResult result = (BoardRequestResult)pnpSimulator.SendMessage(
                IoTTestOrchestrator.OIB.SupportedExtensions.BoardGateKeeper, BoardGateKeeperSendMessage.BoardRequest.ToString(), boarDataToBeSent);
            ...
            var createdBoardProcessedData = OIBUtilities.CreateBoardProcessedData(panel.Name, this.ppResources[1]);
        ...
        {
            var ovenSimulator = (this._smtSimulators["Oven"] as IoTTestOrchestrator.IPCCFX.PluginMain);
            var rand = new Random();
            ...
            var validateUnitsRequest = new CFX.InformationSystem.UnitValidation.ValidateUnitsRequest
            {
                Validations = new List<CFX.Structures.ValidationType>() {
                    CFX.Structures.ValidationType.All
                },
                PrimaryIdentifier = panel.Name,
            };

            ovenSimulator.SendMessage(new CFX.CFXEnvelope(validateUnitsRequest),
                Cmf.Connect.IoT.Driver.IpcCfx.DriverObjects.CommandExecutionMode.SendRequest,
                Cmf.Connect.IoT.Driver.IpcCfx.DriverObjects.CommandExecutionDestination.TargetHandle);

            var messageWasReceived = false;
            ovenSimulator.ResetContext();
            ovenSimulator.WaitForMessage(20, (message) =>
            {
                var cfxMessage = message.Message;

                if (cfxMessage.MessageName == "CFX.InformationSystem.UnitValidation.ValidateUnitsResponse")
                {
            ...
            this._scenario.Utilities.WaitFor(15, "ValidateUnitsResponse was never received", () =>
            {
            ...
            ovenSimulator.SendMessage(new CFX.CFXEnvelope(workStarted),
            ...
            ovenSimulator.SendMessage(new CFX.CFXEnvelope(unitsProcessed),
                Cmf.Connect.IoT.Driver.IpcCfx.DriverObjects.CommandExecutionMode.Publish,
                Cmf.Connect.IoT.Driver.IpcCfx.DriverObjects.CommandExecutionDestination.Exchange);
            ...
            ovenSimulator.SendMessage(new CFX.CFXEnvelope(workCompleted),
                Cmf.Connect.IoT.Driver.IpcCfx.DriverObjects.CommandExecutionMode.Publish,
                Cmf.Connect.IoT.Driver.IpcCfx.DriverObjects.CommandExecutionDestination.Exchange);
