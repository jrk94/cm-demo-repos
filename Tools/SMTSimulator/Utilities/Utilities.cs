...
namespace SMTSimulator
{
    public static class Utilities
    {
        ...
        private static (AutomationControllerInstanceCollection, AutomationDriverInstanceCollection) ParseDriverInstancesFromDataSet(NgpDataSet ngpDataSet)
        {
            ...
            for (int i = 0; i < table.Rows.Count; i++)
            {
                ...
                string typeOfRow = Convert.ToString(table.Rows[i]["EntityTypeName"]);
                if (typeOfRow == "AutomationControllerInstance")
                {
                    ...
                    controller.Name = Convert.ToString(table.Rows[i]["Name"]);
    ...
    public static class OIBUtilities
    {
        public static BoardRequestData CreateBoardRequestData(string barcode, DateTime? boardTime = null)
        {
            return new BoardRequestData()
            {
                Board = new BoardData()
                {
                    Barcode = barcode,
                    BoardTime = boardTime ?? DateTime.Now,
                },
        ...
        public static BoardProcessedData CreateBoardProcessedData(string barcode, string station)
        {
            var data = new BoardProcessedData()
            {
                MessageSendTime = DateTime.UtcNow,
                Station = new Asm.As.Oib.Monitoring.Proxy.Business.Objects.Station()
                {
                    LineFullPath = "Enterprise\\Site1\\Area1\\Line1",
                    FullPath = $"Enterprise\\Site1\\Area1\\Line1\\{station}",
                },
                Recipe = new Asm.As.Oib.Monitoring.Proxy.Business.Objects.Recipe()
                {
                    RecipeName = "Recipe001",
                    RecipeFullPath = "/SiplacePro/Recipes/Recipe001",
                    SetupName = "Setup001",
                    SetupFullPath = "/SiplacePro/Setups/Setup001"
                },
            };

            data.ProcessedBoards.Add(new ProcessedBoard()
            {
                Barcode = barcode,
                BoardSide = 1,
                BuildTimeInt = 120,
                BuildTimePa1 = 100,
                BuildTimePa2 = 20,
                Conveyor = ConveyorType.Left,
            });

            data.UsedDetails.Add(new UsedDetail()
            {
                TrackEntry = new TrackEntry()
                {
                    ComponentName = "R10K",
                    ComponentFullPath = "/SiplacePro/Components/R10K",
                    ComponentShapeName = "Square",
                    ComponentShapeFullPath = "/SiplacePro/Shapes/Square",
                    Track = 10,
                    TableLocation = 2,
                    Tower = 3,
                    Level = 4,
                ...
                FeederId = "Feeder001",
                AccessTotal_Int = 2000,
                TrackEmpty_Int = 500,
                PlacedComponents_Int = 1000,
                RejectIdent_Int = 666,
                RejectVacuum_Int = 750,
                ExtendedRejects = new RejectInfo()
                {
                    IdentError_Int = 333,
                    PickupRetries_Int = 444,
                    TreatmentError_Int = 555,
                }
            });

            data.StationEvents.Add(new StationEvent()
            {
                BoardNumber = 12345,
                StationTime = DateTime.UtcNow,
                StationEventType = StationEventType.PCBEnd2,
                ProcessingArea = 1,
                ProcessingMode = ProcessingModeType.Left,
                Conveyor = ConveyorType.Left,
                SubConveyor = SubConveyorType.Undefined,
            });
            return data;
        }
    }
